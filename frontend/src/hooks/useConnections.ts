...
export const useConnections = () => {
  ...
  useEffect(() => {
    if (!userId) return;
    const ch = supabase
      .channel(`conn-${userId}-${Math.random().toString(36).slice(2)}`)
  ...
  const stateWith = (otherId: string): { state: ConnState; row?: ConnRow } => {
    const row = rows.find((r) => (r.requester_id === otherId && r.recipient_id === userId) || (r.recipient_id === otherId && r.requester_id === userId));
    if (!row) return { state: "none" };
    if (row.status === "accepted") return { state: "accepted", row };
    if (row.status === "declined") return { state: "declined", row };
    return { state: row.requester_id === userId ? "pending_out" : "pending_in", row };
  };

  const pendingIncoming = rows.filter((r) => r.recipient_id === userId && r.status === "pending");
