...
import { Heart, MessageCircle, Pin, Plus, Send, Share2, Sparkles, X } from "lucide-react";
...
export const HomeScreen = () => {
  ...
  const load = async () => {
    const { data } = await supabase
      .from("posts")
      .select("id,content,type,tag,pinned,created_at,author_id,author:profiles!posts_author_id_fkey(name,avatar_url,branch,year,placement_status,company,college_email_verified),likes:post_likes(user_id)")
  ...
  const submitPost = async () => {
    ...
    setDraft(""); setTag(""); setShowCompose(false);
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      {showCompose && (
        <div className="fixed inset-0 z-50 bg-foreground/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 animate-fade-in-up" onClick={() => setShowCompose(false)}>
          <div className="bg-card rounded-3xl p-5 w-full max-w-md shadow-elevated animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={submitPost} disabled={!draft.trim()} className="mt-3 w-full py-3 rounded-2xl bg-gradient-hero text-primary-foreground font-semibold text-sm shadow-glow disabled:opacity-50 flex items-center justify-center gap-2">
